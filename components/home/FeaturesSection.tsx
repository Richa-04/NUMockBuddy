    ...
    <section style={{ padding: '96px 24px', background: 'var(--color-gray-100)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 3.5vw, 48px)', fontWeight: 400, color: 'var(--color-black)', lineHeight: 1.1, letterSpacing: '-0.8px', marginBottom: 16 }}>Practice smarter, not just harder.</h2>
          <p style={{ fontSize: 16, color: 'var(--color-gray-400)', maxWidth: 520, margin: '0 auto', lineHeight: 1.7 }}>Every tool you need, built for Northeastern Seattle students.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
          {FEATURES.map((f) => (
            <div key={f.title} style={{ background: '#fff', border: '1px solid var(--color-gray-200)', borderRadius: 'var(--radius-lg)', padding: 40, display: 'flex', flexDirection: 'column', gap: 16, borderTop: f.accent ? '3px solid var(--color-red)' : '1px solid var(--color-gray-200)' }}>
              <div>
              ...
              <div style={{ padding: '8px 12px', background: 'var(--color-gray-100)', borderRadius: 'var(--radius-md)', fontSize: 12, fontWeight: 500, color: 'var(--color-gray-400)', marginTop: 'auto' }}>{f.highlight}</div>
            </div>
          ))}
        </div>
      </div>
